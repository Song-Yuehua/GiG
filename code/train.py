...
from GAT import GATNet_d, GATNet_p
...
def train_and_evaluate(
    ...
    # ---------------------------------------------------------------------
    # 3) Initialize Models
    model_d      = GATNet_d().to(device)
    model_p      = GATNet_p().to(device)
    model_link   = LinkPredictionModel(num_features=EMBEDDING_DIM, hidden_dim=HIDDEN_DIM).to(device)
    mlp_classifier = MLPClassifier_ab_t(input_dim=EMBEDDING_DIM, hidden_dim=HIDDEN_DIM).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        # -------------------------------------------------------------
        # Use the negative ratio here
        # If neg_ratio=0.3, then num_neg = 0.3 * num_pos
        ...
        preds = mlp_classifier(mlp_input).squeeze() # shape [2*num_pos]
        ...
        if val_auc > best_val_auc:
            ...
            }, f'code/20250416/20250420_GATGATGCN_712_ss/MLP_GCNGCNGAT_20250407_shift_abt_DNS_neg_{neg_ratio}_best_model_by_val_auc.pth')
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'model_link_state_dict': model_link.state_dict(),
                'mlp_classifier_state_dict': mlp_classifier.state_dict()
            }, f'code/20250416/20250420_GATGATGCN_712_ss/MLP_GCNGCNGAT_20250407_shift_abt_DNS_neg_{neg_ratio}_best_model_by_val_loss.pth')
        ...
        if val_aupr > best_val_pr:
            ...
            torch.save({
                ...
                'model_link_state_dict': model_link.state_dict(),
                'mlp_classifier_state_dict': mlp_classifier.state_dict()
            }, f'code/20250416/20250420_GATGATGCN_712_ss/MLP_GCNGCNGAT_20250407_shift_abt_DNS_neg_{neg_ratio}_best_model_by_val_aucpr.pth')
    ...
    checkpoint_auc = torch.load(f'code/20250416/20250420_GATGATGCN_712_ss/MLP_GCNGCNGAT_20250407_shift_abt_DNS_neg_{neg_ratio}_best_model_by_val_aucpr.pth', map_location=device)
    ...
    with torch.no_grad():
        ...
        def visualize_embeddings(embeddings, labels, title="Embeddings Visualization"):
            ...
            plt.savefig(f"code/20250416/20250420_GATGATGCN_712_ss/MLP_GCNGCNGAT_20250407_shift_abt_DNS_neg_{neg_ratio}_features_analysis.png")
            plt.close()
    ...
    final_test_prob = test_results['probs']
    ...
    for idx, th in enumerate(final_thresholds):
        ...
        print('########################')
        count_predicted_positive = np.sum(final_test_prob >= th)
    ...
    # 4) 保存图像
    plt.savefig(f"code/20250416/20250420_GATGATGCN_712_ss/test_roc_curve_{neg_ratio}.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"code/20250416/20250420_GATGATGCN_712_ss/MLP_GCNGCNGAT_20250407_shift_abt_DNS_neg_{neg_ratio}_training_metrics.png")
    plt.close()
...
def evaluate_test_negative_sampling(
    ...
    with torch.no_grad():
        ...
        probs_tset = mlp_classifier(mlp_input).squeeze()

        # 6) BCE loss
        loss = criterion(probs_tset, all_labels).item()

        # 7) Predictions & metrics
        probs = probs_tset.cpu().numpy()  # probabilities in [0,1]
        ...
        visualize_pair_embeddings(
            ...
            out_path=f"code/20250416/20250420_GATGATGCN_712_ss/mlp_input_tsne_by_label_{neg_ratio}.png",
        ...
        visualize_pair_embeddings(
            pair_emb=mlp_input,
            color_values=probs,
            out_path=f"code/20250416/20250420_GATGATGCN_712_ss/mlp_input_tsne_by_prob_{neg_ratio}.png",
...
def evaluate_model_negative_sampling(
    ...
    with torch.no_grad():
        ...
        mlp_input = torch.cat([src_emb, dst_emb], dim=1)
        probs_val = mlp_classifier(mlp_input).squeeze()
...
def main():
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("code/20250416/20250420_GATGATGCN_712_ss/negative_ratio_auc_aupr.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("code/20250416/20250420_GATGATGCN_712_ss/negative_ratio_acc_precision_recall.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("code/20250416/20250420_GATGATGCN_712_ss/negative_ratio_f1_mcc.png")
    plt.close()
